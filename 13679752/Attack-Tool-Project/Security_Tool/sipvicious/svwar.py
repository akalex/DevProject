...
# Class TakeASip : inherits from the base class object. New-style Classes
# Mandatory and optional args can be identified with the __init__ method
class TakeASip:
    #TakeASip Comment : constructor - takes one parameter. This is arguments received from main.py wrapper
                ...
                try:
                    ...
                    if self.sessionpath is not None:
                        if self.packetcount.next():
                            try:
                                if self.guessmode == 1:
                                    pickle.dump(self.nextuser,open(os.path.join(self.sessionpath,'lastextension.pkl'),'w'))
                                    self.log.debug('logged last extension %s' % self.nextuser)
                                elif self.guessmode == 2:
                                    pickle.dump(self.guessargs.tell(),open(os.path.join(self.sessionpath,'lastextension.pkl'),'w'))
...
# Class SVWR : inherits from the base class object. New-style Classes
# Mandatory and optional args can be identified with the __init__ method
class SVWAR(object):

    #SVWAR Comment :  beware if you change anything below - execution starts here
    def main(self):
