...
from threading import Thread, activeCount, enumerate, currentThread, Lock
#from multiprocessing import Lock, Process, current_process, active_children, Pool
import Queue
import re
import random
...
-i <file>
--continue <file>
	This parameter indicates WSP to continue with the scan from the specified
  file, this file should contain data from a previous scan.
	The file is optional, if it is not specified, WSP takes the default file
  from \"scans\" folder.

-k <file>
--attack <file>
	This parameter indicates WSP to perform attacks without scanning again the
...
--help
    ...
    def attack(self):
        ...
        THREAD_LIMIT = 5                # This is how many threads we want
        queue = Queue.Queue(7)          # This sets up the queue object to use 7 slots
        singlelock = Lock()             # This is a lock so threads don't print trough each other (and other reasons)
        ...
        def test(queue):
            while True:
                try:
                    ...
                    item.attack(self.urls, self.forms)
        ...
        for i in xrange(THREAD_LIMIT):
            ...
            worker = Thread(name="Thread-%s" % i, target=test, args=(queue,))
            worker.setDaemon(True)
            worker.start()

        for inst in self.attacks:
            if inst.doGET == False and inst.doPOST == False:
                continue
            print
            if inst.require != []:
                t = [y.name for y in self.attacks if y.name in inst.require and (y.doGET or y.doPOST)]
                if inst.require != t:
                    print "[!]", _("Missing dependecies for module"), inst.name, ":"
                    print "  ", ",".join([y for y in inst.require if y not in t])
                    continue
                else:
                    inst.loadRequire([y for y in self.attacks if y.name in inst.require])
            print ' Queuing:', inst.name
            try:
                queue.put(inst, block=True, timeout=1)
            except:
                singlelock.acquire()
        ...
        singlelock.release()        # Release the lock
        queue.join()                # This command waits for all threads to finish.
        print ' Done'
    ...
    try:
        ...
        if attackFile is not None:
            if crawlerPersister.isDataForUrl(attackFile) == 1:
                ...
                # TODO: xml structure
                wsp.urls = crawlerPersister.getBrowsed()
                wsp.forms = crawlerPersister.getForms()
                # wsp.uploads = crawlerPersister.getUploads()
                print _("File") + " " + attackFile + " " + \
                      _("loaded, WSP will use it to perform the attacks")
            else:
                print _("File") + " " + attackFile + " " + \
                      _("not found, WSP will scan again the web site")
                wsp.browse(crawlerFile)
        else:
            wsp.browse(crawlerFile)
        try:
            wsp.attack()
            ...
            print ""
            print _(
                "Attack process interrupted. To perform again the attack, lauch WSP with \"-i\" or \"-k\" parameter.")
            print ""
            pass
    except SystemExit:
        pass
