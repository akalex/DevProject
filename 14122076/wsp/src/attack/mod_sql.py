...
class mod_sql(Attack):
    ...
    def __findPatternInResponse(self, data):
        ...
        if data.find("supplied argument is not a valid ldap") >= 0 or data.find(
                "javax.naming.NameNotFoundException") >= 0:
    ...
    def attackGET(self, page, dict, headers={}):
        ...
        if dict == {}:
            ...
            for k in dict.keys():
                ...
                tmp = dict.copy()
                tmp[k] = "__PAYLOAD__"
                url = page + "?" + self.HTTP.encode(tmp, headers["link_encoding"]).replace("__PAYLOAD__",
                ...
                if url not in self.attackedGET:
                    ...
                    if err != "":
                        ...
                        self.reportGen.logVulnerability(Vulnerability.SQL_INJECTION,
                                                        Vulnerability.HIGH_LEVEL_VULNERABILITY,
                                                        url, self.HTTP.encode(tmp, headers["link_encoding"]).replace(
                                "__PAYLOAD__", self.HTTP.quote(payload)),
                                                        err + " (" + k + ")")
                        if self.color == 0:
