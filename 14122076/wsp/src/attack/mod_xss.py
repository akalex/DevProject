...
class mod_xss(Attack):
    ...
    # form the simplest to the most complex, WSP will stop on the first working
    ...
    def attackGET(self, page, dict, headers={}):
        ...
        if dict == {}:
            ...
            if url not in self.attackedGET:
                ...
                err = ""
                code = "".join([random.choice("0123456789abcdefghjijklmnopqrstuvwxyz") for i in
                                range(0, 10)])  # don't use upercase as BS make some data lowercase
                url = page + "?" + code
    ...
    def validXSS(self, page, code):
        ...
        for x in soup.findAll("script"):
            if x.string is not None and x.string in [t.replace("__XSS__", code) for t in self.script_ok]:
                return True
            elif x.has_key("src"):
            ...
            if self.validXSS(dat, code):
                ...
                if referer != "":
                    ...
                    if self.color == 0:
                        ...
                        print "  " + _("with params") + " =", self.HTTP.encode(params, encoding).replace(var + "=",
                                                                                                         self.RED + var + self.STD + "=")
            ...
            if self.validXSS(dat, code):
                ...
                if referer != "":
                    ...
                    if self.color == 0:
                        ...
                        print "  " + _("with params") + " =", self.HTTP.uqe(params, encoding).replace(var + "=",
                                                                                                      self.RED + var + self.STD + "=")
                    print "  " + _("coming from"), referer
