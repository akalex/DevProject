...
'''
api.py -  vulnDB - Open Source Cross-linked and Aggregated Local Vulnerability Database
...
class vulnDB(object):
    ...
    def __init__(self, cveID):
        ...
        self.vulndb_db = os.path.join('vulndb', config.database['vulndb_db'])
        self.vulndb_db_url = config.database['primary']['url']
        ...
        self._check_env(self.vulndb_db)
        self._db_init()
        self._vrfy_cve()
    ...
    def _vrfy_cve(self):
        try:
            ...
            if self.data is None:
                print '[warning] Entry %s is missed from vulnDB Database' % self.cveID
                print '[hint] Update your local vulndb.db'
                exit(0)
        except Exception, e:
            print '[exception]:', e
            exit(0)
    ...
    def _check_env(self, file):
        ...
        if not os.path.isfile(file):
            ...
            print '[db_error] try python vulndbcli.py update '
            exit(0)

    def _db_init(self):

        try:
            self.conn = sqlite3.connect(self.vulndb_db)
            self.cur = self.conn.cursor()
            self.query = (self.cveID,)
            return (self.cur, self.query)
        except Exception, e:
            print '[error] something occurred while opening the database', self.vulndb_db
            print '[exception]:', e
            exit(0)
